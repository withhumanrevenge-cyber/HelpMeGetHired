    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
        <div className="text-center mb-10 sm:mb-14">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight">Simple pricing</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {PRICING_TIERS.map((plan) => (
            <div key={plan.name}
              className={`rounded-lg border p-6 sm:p-7 flex flex-col ${plan.featured ? "border-gray-900 bg-gray-900" : "border-gray-200 bg-white"}`}>
              ...
              <div className="flex items-end gap-1 mb-1">
                <span className={`text-4xl font-bold ${plan.featured ? "text-white" : "text-gray-900"}`}>{plan.priceLabel}</span>
        ...
        <p className="text-center text-xs text-gray-400 mt-6">{CREDIT_SUMMARY}</p>
        {currency === "INR" && (
          <p className="text-center text-[11px] text-gray-400 mt-1.5">Prices shown in ₹ for India · pay by UPI or card. International pricing in USD.</p>
        )}
      ...
      <footer className="border-t border-gray-100 mt-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 flex justify-between items-center text-xs text-gray-400">
